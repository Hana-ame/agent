...
".join(lines)
 ...
 # 生成预览
 if len(stripped_content) > PREVIEW_LENGTH * 2:
 preview = stripped_content[:PREVIEW_LENGTH] + "...(中间省略)..." + stripped_content[-PREVIEW_LENGTH:]
 else:
 preview = stripped_content
 results.append(f"{rel_path}中被写入了以下内容
{preview}")
 except Exception as e:
 results.append(_handle_error(rel_path, str(e)))
