...
class ChatCLI:
    ...
    DEFAULT_PAYLOAD = "payloads/payload.json"

    def _resolve_config_path(self, path: str) -> str:
        """【调用效果】尝试在当前目录、payloads/ 目录中寻找配置文件，且禁止使用 archive 目录"""
        p = Path(path)
        resolved_p = p.resolve()
        
        if "archive" in resolved_p.parts:
            raise PermissionError(f"禁止加载 archive 目录中的配置文件: {path}")

        if p.exists():
            return str(resolved_p)
        
        # 尝试在 payloads/ 目录下寻找 (针对 payload 文件)
        payloads_path = Path("payloads") / p.name
        if payloads_path.exists():
            resolved_payloads = payloads_path.resolve()
            if "archive" in resolved_payloads.parts:
                raise PermissionError(f"禁止加载 archive 目录中的配置文件: {path}")
            return str(resolved_payloads)
    ...
    def __init__(
        ...
        *,
        ...
        ...
        【参数说明】
            history_file: 对话历史 JSON 文件路径
            endpoint_file: API 配置覆盖
            endpoint_name: 指定使用哪个 endpoint (如 'official', 'moonchan')
            payload_file: Payload 模板覆盖
            ...
        """
        ...
        # 文件路径配置
        self._endpoint_file = self._resolve_config_path(endpoint_file or self.DEFAULT_ENDPOINT)
        self._payload_file = self._resolve_config_path(payload_file or self.DEFAULT_PAYLOAD)
...
示例:
  ...
  %(prog)s -d doc.txt "分析" --new   # 新会话+文档分析
    ...
    parser.add_argument(
        '--endpoint-name',
        metavar='NAME',
        help='指定使用配置文件中的哪个 endpoint 标识符 (例如: official, moonchan)'
    )
