...
class ReviewerOrganelle(BaseOrganelle):
    ...
    def estimate_quality(self, result: ExecutionResult, task_input: dict) -> float:
        ...
        review_markers = ["issue", "suggestion", "improve", "bug", "error", "warning", "fix"]
        matches = sum(1 for m in review_markers if m in result.output.lower())
