#!/usr/bin/env python3
"""HN AI Report — local run with the real (free) LLM.

Runs ``config.json``: fetch top HN stories -> LLM filter -> ProcessStoriesMap
(fetch comments + summarize) -> report.md.

The endpoint (complete URL incl. /chat/completions) and proxy are declared
explicitly in config.json settings — the framework does NOT fallback-fill an
endpoint. Set HTTPS_PROXY only as a transport fallback; the config proxy wins.
