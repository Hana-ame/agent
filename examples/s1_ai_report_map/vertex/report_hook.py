...
- Override: set env ``S1_REPORT_OUT`` to a target file path. The chatto-bot
  s1_report plugin uses this so the bot keeps its own report copy and never
...
class ReportVertex(Vertex):
    ...
    def on_receive(self, data, channel, settings):
        ...
        out = os.environ.get("S1_REPORT_OUT")
        if out:
