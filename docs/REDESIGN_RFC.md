...
| F | Standalone edge CLI flag names match the config JSON keys: `--id` replaced `--edge-id` (kept as a deprecated alias — `tests/test_v4_system.py` still uses the old spelling), `--input` / `--output` keep their short names but the help text now names the JSON keys they map onto (`input_vertex` / `output_vertex`), and the canonical config key for the session is `"session"` (`"session_id"` kept as a deprecated alias), and seeding is now `--seed` / `"seed"` (`--seed-input` / `"seed_input"` kept as a deprecated alias). `--db` / `--seed` / `--mock` / `--dir` are runner parameters — consumed by the CLI itself, not by `EdgeV4.from_config` — which is why they only appear in a single-edge `--config`, never in a graph manifest | `framework/edges/cli.py`, `tests/test_edges_cli_type.py` | usability follow-up |
