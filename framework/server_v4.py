...
def create_v4_server(
    ...
    # -----------------------------------------------------------------------
    # Edge Metrics & Performance Benchmarks API
    # -----------------------------------------------------------------------
    ...
    @app.get("/api/db/metrics/summary")
    async def get_global_metrics_summary() -> Dict[str, Any]:
    ...
    #
    # Integrates directly with the V4 engine, providing:
    # - Persistent session state via SessionGraphManagerV4 + SQLite
    # - Hop-by-hop execution via tool_calls (advance_graph)
    # - Full execution mode (runs graph to completion)
    # - SSE streaming with OpenAI-format delta chunks
    # -----------------------------------------------------------------------
    ...
    app.state.register_graph_template = _register_graph_template
    ...
    async def openai_chat_completions(
        ...
        async with manager.get_session_lock(session_id):
            ...
            if has_tool_edges or is_tool_reply or execution_mode in ("harness", "step"):
                from framework.http_executor_v4 import HttpHarnessExecutorV4
                http_exec = HttpHarnessExecutorV4(store=store, agent=app.state.agent)
...
# Module-level app for ``uvicorn framework.server_v4:app``
