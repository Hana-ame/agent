"""V4 Chat Protocol LLM Edge.

Subclass of LLMEdgeV4 dedicated to chat completion agent invocations.
"""
...
class ChatLLMEdgeV4(LLMEdgeV4):
    """Executes LLM inference via agent.chat([{role: 'user', content: ...}], model=..., temperature=...)."""
    ...
    async def _invoke_agent(
        ...
        if not (hasattr(agent, "chat") and callable(agent.chat)):
