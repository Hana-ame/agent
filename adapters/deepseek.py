...
class DeepSeekParser:
    ...
    # version: 0.0.2
    # 上下文：每次接收到通过 browser 频道透传的 payload.data 时触发。先决调用：XHR 劫持到新 Chunk 并通过 Server 转达。后续调用：外层通过返回值判断是否断开循环。
    # 输入参数：data (dict) 对应 JS 端拦截到的 JSON 对象
    # 输出参数：is_done (bool)
    def on_message(self, data: dict) -> bool:
        # V2: 提前判断 Server 标记结束的特征
